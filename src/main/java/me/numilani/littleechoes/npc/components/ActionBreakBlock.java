...
public class ActionBreakBlock extends ActionBase {
  ...
  public ActionBreakBlock(@Nonnull BuilderActionBreakBlock builder, @Nonnull BuilderSupport support) {
    super(builder);
  ...
  public boolean canExecute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider sensorInfo,
    ...
    if (super.canExecute(ref, role, sensorInfo, dt, store) && sensorInfo != null && sensorInfo.hasPosition()) {
      BlockType placedBlockType = BlockType.getAssetMap().getAsset("Furniture_Temple_Light_Statue"); // TODO: this is a
                                                                                                     // bad workaround
      if (placedBlockType == null) {
        ...
        if (transformComponent.getPosition().distanceSquaredTo(x, y, z) > maxDistance * maxDistance) {
          return false;
        } else if (sensorInfo instanceof CachedPositionProvider
            && !((CachedPositionProvider) sensorInfo).isFromCache()) {
          return true;
        } else {
  ...
  public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, InfoProvider sensorInfo, double dt,
    ...
    var blockType = chunk.getBlockType(MathUtil.floor(this.target.getX()), MathUtil.floor(this.target.getY()),
        MathUtil.floor(this.target.getZ()));
    chunk.breakBlock(MathUtil.floor(this.target.getX()), MathUtil.floor(this.target.getY()),
        MathUtil.floor(this.target.getZ()));

    ItemStack drop = InventoryHelper.createItem(blockType.getItem().getId());
    if (drop != null) {
      // ItemUtils.throwItem(ref, store, drop, new Vector3d(0,0,0), 0);
      var npc = (LivingEntity)EntityUtils.getEntity(ref, store);
      var inv = npc.getInventory().getStorage();
      if (inv.canAddItemStack(drop)){
        inv.addItemStack(drop);
      }
    }
